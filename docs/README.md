...
**前端核心**:
- **Vue 3.4.0** + Composition API + TypeScript支持
- **Vite 4.5.0** - 快速构建和热重载
- **Tailwind CSS 3.3.6** + PostCSS 8.4.32 - 原子化CSS框架
- **Vue Router 4.2.5** - 单页应用路由管理
- **Pinia 3.0.3** - Vue 3官方状态管理

**开发和测试**:
- **Vitest 0.34.6** + **Vue Test Utils 2.4.6** - 现代化测试框架
- **@testing-library/vue 7.0.0** - 用户行为导向测试
- **JSDOM 22.1.0** - 浏览器环境模拟
- **@vitest/ui** - 可视化测试界面

**后端和服务**:
- **Supabase 2.54.0** - PostgreSQL数据库 + 实时订阅 + RLS安全
- **Vercel Serverless Functions** - 边缘计算API
- **@vercel/node 5.3.11** - Vercel运行时支持

**云服务集成**:
- **阿里云OSS** - ali-oss 6.23.0 (文件存储和CDN)
- **阿里云SMS** - @alicloud/pop-core 1.8.0 (短信验证)
- **Remove.bg API** - AI背景移除服务
- **VanceAI API** - AI图片放大服务

**国际化和工具**:
- **Vue I18n 9.14.5** - 中英文国际化支持
...
│   │   │   ├── EmptyImagePrompt.vue      # 空图片提示
│   │   │   ├── ImageCanvas.vue           # 图片画布
...
│   │   └── Help.vue            # 帮助页面
│   └── utils/                  # 工具函数
│       ├── aiServices.js       # AI服务工具
│       ├── formatUtils.js      # 格式化工具
│       ├── i18nNotification.js # 国际化通知工具
│       ├── imageUtils.js       # 图片处理工具
│       ├── ossClient.js        # OSS客户端工具
│       ├── phoneUtils.js       # 手机号工具
...
│   │   └── verify-sms/         # 短信验证函数
│   ├── migrations/             # 数据库迁移文件
│   │   ├── 002_sms_verification_codes.sql    # 短信验证表
│   │   ├── 003_fix_rls_policies.sql          # RLS策略修复
│   │   ├── 004_add_usage_events.sql          # 使用事件表
│   │   └── 005_add_consume_credits_function.sql # 配额消费函数
│   ├── config.toml             # Supabase项目配置
│   └── .gitignore              # Supabase忽略文件
├── tests/                       # 🧪 测试框架
│   ├── unit/                   # 单元测试
│   │   ├── components/         # 组件测试
│   │   ├── composables/        # 组合函数测试
...
├── postcss.config.js            # PostCSS根配置
├── tailwind.config.js           # Tailwind根配置
...
| `npm run test-private` | 私有OSS配置测试脚本 |
